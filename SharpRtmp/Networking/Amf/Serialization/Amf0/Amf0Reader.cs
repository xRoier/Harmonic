...
using SharpRtmp.Networking.Amf.Data;
using SharpRtmp.Networking.Amf.Serialization.Amf3;
using SharpRtmp.Networking.Amf.Serialization.Attributes;
...
{
    private readonly IReadOnlyDictionary<Amf0Type, long> _typeLengthMap = new Dictionary<Amf0Type, long>
    {
        { Amf0Type.Number, 8 },
        { Amf0Type.Boolean, sizeof(byte) },
        { Amf0Type.String, Amf0CommonValues.StringHeaderLength },
        { Amf0Type.LongString, Amf0CommonValues.LongStringHeaderLength },
        { Amf0Type.Object, /* object marker*/ Amf0CommonValues.MarkerLength - /* utf8-empty */Amf0CommonValues.StringHeaderLength - /* object end marker */Amf0CommonValues.MarkerLength },
        { Amf0Type.Null, 0 },
        ...
        { Amf0Type.TypedObject, /* object marker*/ Amf0CommonValues.MarkerLength - /* class name */ Amf0CommonValues.StringHeaderLength - /* at least on character for class name */ 1 - /* utf8-empty */Amf0CommonValues.StringHeaderLength - /* object end marker */Amf0CommonValues.MarkerLength },
    ...
    public void ResetReference() => _referenceTable.Clear();
    ...
    public void RegisterIExternalizableForAvmPlus<T>() where T : IExternalizable, new() => _amf3Reader.RegisterExternalizable<T>();
    ...
    private bool TryReadHeader(Span<byte> buffer, out KeyValuePair<string, object> header, out int consumed)
    {
        header = default;
        consumed = 0;
        if (!TryGetStringImpl(buffer, Amf0.Amf0CommonValues.StringHeaderLength, out var headerName, out var nameConsumed))
            return false;
    ...
    public bool TryGetMessage(Span<byte> buffer, out Message message, out int consumed)
    {
        ...
        if (!TryGetStringImpl(buffer, Amf0CommonValues.StringHeaderLength, out var targetUri, out var targetUriConsumed))
            return false;

        buffer = buffer[targetUriConsumed..];
        if (!TryGetStringImpl(buffer, Amf0CommonValues.StringHeaderLength, out var responseUri, out var responseUriConsumed))
            return false;
    ...
    public bool TryDescribeData(Span<byte> buffer, out Amf0Type type, out int consumedLength)
    {
        ...
        if (buffer.Length < Amf0CommonValues.MarkerLength)
            return false;

        var marker = (Amf0Type)buffer[0];
        if (!_typeLengthMap.TryGetValue(marker, out var bytesNeed))
            return false;
        if (buffer.Length - Amf0CommonValues.MarkerLength < bytesNeed)
            return false;

        type = marker;
        consumedLength = (int)bytesNeed + Amf0CommonValues.MarkerLength;
    ...
    public bool TryGetNumber(Span<byte> buffer, out double value, out int bytesConsumed)
    {
        ...
        if (type != Amf0Type.Number)
            return false;
        value = NetworkBitConverter.ToDouble(buffer[Amf0CommonValues.MarkerLength..]);
    ...
    public bool TryGetString(Span<byte> buffer, out string value, out int bytesConsumed)
    {
        ...
        if (!TryGetStringImpl(buffer[Amf0CommonValues.MarkerLength..], Amf0CommonValues.StringHeaderLength, out value, out bytesConsumed))
            return false;

        bytesConsumed += Amf0CommonValues.MarkerLength;
    ...
    private bool TryGetObjectImpl(Span<byte> objectBuffer, out Dictionary<string, object> value, out int bytesConsumed)
    {
        ...
        while (true)
        {
            if (!TryGetStringImpl(objectBuffer, Amf0CommonValues.StringHeaderLength, out var key, out var keyLength))
                return false;
            consumed += keyLength;
    ...
    public bool TryGetObject(Span<byte> buffer, out AmfObject value, out int bytesConsumed)
    {
        ...
        var objectBuffer = buffer[Amf0CommonValues.MarkerLength..];

        if (!TryGetObjectImpl(objectBuffer, out var obj, out var consumed))
            return false;

        value = new AmfObject(obj);
        bytesConsumed = consumed + Amf0CommonValues.MarkerLength;
    ...
    public bool TryGetUndefined(Span<byte> buffer, out Undefined value, out int consumedLength)
    {
        ...
        if (type != Amf0Type.Undefined)
            return false;
        value = new Undefined();
        consumedLength = Amf0CommonValues.MarkerLength;
        return true;
    }
    ...
    private bool TryGetReference(Span<byte> buffer, out object value, out int consumedLength)
    {
        ...
        var index = NetworkBitConverter.ToUInt16(buffer.Slice(Amf0CommonValues.MarkerLength, sizeof(ushort)));
        consumedLength = Amf0CommonValues.MarkerLength + sizeof(ushort);
    ...
    private bool TryGetKeyValuePair(Span<byte> buffer, out KeyValuePair<string, object> value, out bool kvEnd, out int consumed)
    {
        ...
        consumed = 0;
        if (!TryGetStringImpl(buffer, Amf0CommonValues.StringHeaderLength, out var key, out var keyLength))
            return false;
        consumed += keyLength;
    ...
    public bool TryGetEcmaArray(Span<byte> buffer, out Dictionary<string, object> value, out int consumedLength)
    {
        ...
        var elementCount = NetworkBitConverter.ToUInt32(buffer.Slice(Amf0CommonValues.MarkerLength, sizeof(uint)));

        var arrayBodyBuffer = buffer[(Amf0CommonValues.MarkerLength + sizeof(uint))..];
        var consumed = Amf0CommonValues.MarkerLength + sizeof(uint);
        if (StrictMode)
        {
            ...
            if (!TryGetStringImpl(arrayBodyBuffer, Amf0CommonValues.StringHeaderLength, out var emptyStr, out var emptyStrConsumed))
                return false;
            if (emptyStr.Any())
                return false;
    ...
    public bool TryGetDate(Span<byte> buffer, out DateTime value, out int consumendLength)
    {
        ...
        var timestamp = NetworkBitConverter.ToDouble(buffer[Amf0CommonValues.MarkerLength..]);
    ...
    public bool TryGetLongString(Span<byte> buffer, out string value, out int consumedLength)
    {
        ...
        if (!TryGetStringImpl(buffer[Amf0CommonValues.MarkerLength..], Amf0CommonValues.LongStringHeaderLength, out value, out consumedLength))
            return false;
    ...
    private bool TryGetStringImpl(Span<byte> buffer, int lengthOfLengthField, out string value, out int consumedLength)
    {
        value = default;
        consumedLength = default;
        var stringLength = lengthOfLengthField == Amf0CommonValues.StringHeaderLength
            ? NetworkBitConverter.ToUInt16(buffer)
            : (int)NetworkBitConverter.ToUInt32(buffer);
    ...
    public bool TryGetXmlDocument(Span<byte> buffer, out XmlDocument value, out int consumedLength)
    {
        value = default;
        consumedLength = default;
        if (!TryDescribeData(buffer, out _, out _))
            return false;

        if (!TryGetStringImpl(buffer[Amf0CommonValues.MarkerLength..], Amf0CommonValues.LongStringHeaderLength, out var str, out consumedLength))
            return false;
    ...
    public bool TryGetTypedObject(Span<byte> buffer, out object value, out int consumedLength)
    {
        ...
        var consumed = Amf0CommonValues.MarkerLength;

        if (!TryGetStringImpl(buffer[Amf0CommonValues.MarkerLength..], Amf0CommonValues.StringHeaderLength, out var className, out var stringLength))
            return false;
        ...
        foreach (var (name, setter) in state.Members)
    ...
    private bool TryGetAvmPlusObject(Span<byte> buffer, out object value, out int consumed)
    {
        value = default;
        consumed = default;
        ...
        buffer = buffer.Slice(Amf0CommonValues.MarkerLength);

        if (!_amf3Reader.TryGetValue(buffer, out value, out consumed))
            return false;

        consumed += Amf0CommonValues.MarkerLength;
    ...
    public bool TryGetValue(Span<byte> objectBuffer, out Amf0Type objectType, out object data, out int valueLength)
    {
        ...
        valueLength = default;
        objectType = default;
        if (!TryDescribeData(objectBuffer, out var type, out _))
            return false;

        if (type == Amf0Type.ObjectEnd)
        {
            objectType = type;
            valueLength = Amf0CommonValues.MarkerLength;
            return true;
        }
